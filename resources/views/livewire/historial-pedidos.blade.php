            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 overflow-hidden relative">
                ...
                <div class="flex justify-between items-center mb-4">
                    <div>
                        ...
                        <span class="text-lg font-bold text-gray-900 dark:text-white">{{ strtoupper(substr($pedido->id_pedido, 0, 8)) }}</span>
                    </div>
                    <div class="text-right">
                        <span class="text-xl font-black text-red-600 dark:text-red-400">${{ $pedido->total_precio }}</span>
                    </div>
                </div>
                ...
                <div class="flex justify-between items-center mb-6">
                    <span class="font-bold {{ $pedido->estado == 'pendiente' ? 'text-gray-600 dark:text-gray-300' : 'text-gray-400' }}">⏳ Recibido</span>
                    <span class="font-bold {{ $pedido->estado == 'en_preparacion' ? 'text-yellow-600 dark:text-yellow-400' : 'text-gray-400' }}">🍳 Preparando</span>
                    <span class="font-bold {{ $pedido->estado == 'lista' ? 'text-green-600 dark:text-green-400 text-lg animate-bounce' : 'text-gray-400' }}">✅ ¡Listo!</span>
                </div>
